...
### How QGS gets PCK certificates

The Intel TDX DCAP operator handles PCK certificate caching automatically.
You do NOT need to configure QGS to reach PCCS — the operator uses a
different mechanism:

1. The QGS DaemonSet's initContainer (`pck-cert-tool`) collects the
   platform manifest from TDX hardware and creates a Kubernetes secret
2. The `registrar` deployment watches for platform secrets and fetches
   PCK certificates from Intel PCS (this step requires internet access
   during initial deployment, or pre-created `-pck` secrets for offline)
3. The registrar creates a `-pck` secret with the PCK certificate data
4. A sidecar in the QGS pod writes the cert data to a local cache at
   `/run/dcap/cache/.dcap-qcnl/`
